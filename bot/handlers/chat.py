...
from bot.services.llm import stream_reply
...
_streaming_users: set[int] = set()

EDIT_EVERY_CHARS = 30
MIN_EDIT_INTERVAL = 0.3
...
async def handle_text_message(message: Message, session: AsyncSession):
    ...
    try:
        ...
        is_safe, rejection = await check_content(message.text)
        if not is_safe:
            await message.reply(rejection)
            return

        await save_msg(session, user.id, "user", message.text)
        ...
        try:
            ...
            error_text = "Слишком много запросов к API. Подождите немного."
            if sent is not None:
                try:
                    await sent.edit_text(error_text)
                except Exception:
